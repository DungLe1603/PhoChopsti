...
//
function article($act, $typeFunc, $article_id, $article_menu_id, $name, $slug, $title, $description, $keywords, $img, $img_note, $comment, $content, $is_active, $hot, $created_time, $upload_img_id, $error, $link, $linhvuc, $tamnhin, $thanhtich, $doitac) {
	global $db, $tth;
	$category = 0;
				...
				<form action="<?php echo $act?>" method="post" enctype="multipart/form-data">
					...
					<table class="table table-hover">
						...
						<tr>
							...
							<td width="12%" align="right" class="ver-top"><label><?php if($category==137) echo 'Giá:';else echo 'Ghi chú hình:';?></label></td>
						...
						<?php if($category==136){?>
							<tr>
								<td width="12%" align="right" class="ver-top"><label>Link đối tác:</label></td>
								<td width="88%" colspan="3">
									<input class="form-control" type="text" name="link" maxlength="255" value="<?php echo stripslashes($link)?>">
								</td>
							</tr>
						<?php } ?>
						<?php if($category==10 || $category == 135){ }else{?>
							<tr>
								<td align="right" class="ver-top"><label><?php if($category==129) echo 'Lời ngỏ:';else echo 'Mô tả:';?></label></td>
								<td colspan="3">
									<textarea class="form-control" rows="3" id="comment" name="comment"><?php echo stripslashes($comment)?></textarea>
								</td>
							</tr>
						<?php
							} if($category==129 || $category==137){
						?>
						...
						<tr>
							<td align="right" class="ver-top">
								<label><?php if($category==129) echo 'Về chúng tôi:';else echo 'Nội Dung Chi Tiết:';?></label>
							</td>
						...
						?>
							<tr>
								...
								<td colspan="3"><textarea class="form-control" name="linhvuc" id="linhvuc"><?php echo stripslashes($linhvuc)?></textarea></td>
							</tr>
							<tr>
								<td align="right" class="ver-top">
									<label>Tầm nhìn - sứ mệnh</label>
								</td>
								<td colspan="3"><textarea class="form-control" name="tamnhin" id="tamnhin"><?php echo stripslashes($tamnhin)?></textarea></td>
							</tr>
							<tr>
								<td align="right" class="ver-top">
									<label>Thành tích</label>
								</td>
								<td colspan="3"><textarea class="form-control" name="thanhtich" id="thanhtich"><?php echo stripslashes($thanhtich)?></textarea></td>
							</tr>
							<tr>
								<td align="right" class="ver-top">
									<label>Đối tác</label>
								</td>
								<td colspan="3"><textarea class="form-control" name="doitac" id="doitac"><?php echo stripslashes($doitac)?></textarea></td>
							</tr>
							<script type="text/javascript">
								CKEDITOR.replace('linhvuc', {
									height: 300}
								);
								CKEDITOR.replace('tamnhin', {
									height: 300}
								);
								CKEDITOR.replace('thanhtich', {
									height: 300}
								);
								CKEDITOR.replace('doitac', {
									height: 300}
								);
							</script>
						<?php } ?>
