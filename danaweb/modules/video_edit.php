			...
			for($i=0;$i<=count($link);$i++)
			{
				...
				if ( (strpos($y_link,"youtube.com") != false) && ($valid != NULL) ) {
					...
					if($OK) {
						...
						$img = $filename;
						$imgVideo = new Upload($dir_dest . $img . '.jpg');
						$imgVideo->file_new_name_body      = 'gal-' . $img;
						$imgVideo->image_resize            = true;
						$imgVideo->image_ratio_crop        = true;
						$imgVideo->image_x                 = 455;
						$imgVideo->image_y                 = 330;
						$imgVideo->Process($dir_dest);
						if($imgVideo->processed) {
							$img = $imgVideo->file_dst_name;
						}
...
if(!$OK) video("?".TTH_PATH."=video_edit", "edit", $gallery_id, $gallery_menu_id, $name, $slug, $description, $keywords, $img, $link_youtube, $comment, $is_active, $hot, $created_time, $error);
?>