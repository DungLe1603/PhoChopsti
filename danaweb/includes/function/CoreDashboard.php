...
//----------------------------------------------------------------------------------------------------------------------
/**
 * Cập nhật quyền quản trị: Quản lý nội dung > Hỏi đáp
 * @param $role_id
 */
function showCoreQuestion ($role_id) {
	global $db;

	$privilege = array();
	$db->table = "core_privilege";
	$db->condition = "role_id = ".$role_id. " and type = 'question'";
	$db->order = "";
	$db->limit = "";
	$rows = $db->select();
	$stt = 0;
	...
	<div class="form-group">
		<div style="padding: 10px; border-left: 1px solid #ddd;">
			<div class="checkbox">
				<label>
					<input type="checkbox" id="selecctallQuestion" ><b>Chọn tất cả / Hủy tất cả</b>
				</label>
			</div>
			<div class="checkbox">
				<label>
					<input type="checkbox" class="checkboxQuestion" name="variable[]" <?=(in_array("question_add",$privilege)) ? "checked" : "" ?> value="question_add">Thêm trang
				</label>
			</div>
			<div class="checkbox">
				<label>
					<input type="checkbox" class="checkboxQuestion" name="variable[]" <?=(in_array("question_edit",$privilege)) ? "checked" : "" ?> value="question_edit">Chỉnh sửa trang
				</label>
			</div>
			<div class="checkbox">
				<label>
					<input type="checkbox" class="checkboxQuestion" name="variable[]" <?=(in_array("question_del",$privilege)) ? "checked" : "" ?> value="question_del">Xóa trang
				</label>
			</div>
		</div>
		<label><button type="submit" class="btn btn-form-primary btn-form">Xác nhận</button></label>
	...
	<script>
		$(document).ready(function() {
			$('#selecctallQuestion').click(function(event) {
				if(this.checked) {
					$('.checkboxQuestion').each(function() {
						this.checked = true;
					});
				}else{
					$('.checkboxQuestion').each(function() {
