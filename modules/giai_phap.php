...
?>
<div class="container-fluid fluid_slilhe">
	<div class="row">
		<?php
			$db->table = "gallery";
		...
		?>
			<img src="<?php echo HOME_URL;?>/uploads/gallery/full_<?php echo $rowh['img'];?>">
		<?php } ?>
		<img src="<?php echo HOME_URL;?>/images/gach.png" class="gach_png">
		<p class="ctac_inslider"><?php echo $gp;?></p>
	</div>
</div>
<?php 
//-------------------------------------------------------------------------------
if ($id_article > 0){
	$id = $id_article;
	include(_F_TEMPLATES . DS . "show_article_giai_phap.php");
} else if($id_menu <= 0) {
	$loc = array();
	$db->table = "article_menu";
	...
	if($total>1) {
		...
		$db->table = "article";
		$db->condition = "is_active = 1 and hot = 0 and article_menu_id IN (".$loc.")";
		$db->order = "created_time DESC";
		$db->limit = $start.','.$per_page;
		$rows = $db->select();
		$i = 0; 
			include(_F_TEMPLATES . DS . "show_list_article_giai_phap.php");
